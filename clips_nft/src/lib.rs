...
impl ClipsNftContract {
    ...
    pub fn create_listing(env: Env, mut listing: ListingRequest) -> Result<ListingId, Error> {
        ...
        listing_storage::create_listing(&env, &mut listing)
    }
    ...
    pub fn complete_listing(env: Env, buyer: Address, token_id: TokenId) -> Result<(), Error> {
        ...
        // Load the active listing so the sale can be recorded in the
        // `nft_sold` event before the listing is removed from storage (#925).
        let listing = listing_storage::get_listing(&env, token_id)?;

        nft_sold_event::emit_nft_sold(
            &env,
            listing.listing_id,
            token_id,
            &listing.seller,
